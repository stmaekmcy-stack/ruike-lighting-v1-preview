...
正式网站最终通过生产运行 `35506459749` 上线，公网版本为 `be80b05819053ea0ec99af35f78e5312c5b98cf3`，`PRODUCTION_ALLOW_INDEXING=true`；HTTPS、续期、路由、索引隔离及 1440/390/430 浏览器交互均已实际验证。443 云防火墙缺失已修复。下文首次引导只供新服务器参考，当前服务器不得重复初始化。用户已明确授权正式上线，不再重复要求固定确认口令；预览始终保持 noindex。无真实接收端则不启用表单。回滚前版本为 `0cbe1fc9396604ffe46195e4d54350889244c688`。
