...
正式网站已通过生产运行 `35506036575` 上线，公网版本为 `0cbe1fc9396604ffe46195e4d54350889244c688`；HTTPS、续期、路由及 1440/390/430 浏览器交互均已实际验证。443 云防火墙缺失已修复。下文首次引导只供新服务器参考，当前服务器不得重复初始化。用户已明确授权正式上线，不再重复要求固定确认口令；索引开启必须在公网验收之后，预览保持隔离。无真实接收端则不启用表单。
